...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const cacheKey = `aqi:${cityId || 'all'}:${period}`;
    ...
    const stats = {
      ...
      average: aqiValues.length ? Math.round(aqiValues.reduce((a, b) => a + b, 0) / aqiValues.length) : 0,
    ...
    const ttl = period === '24h' ? CACHE_TTL.AQI_CURRENT : CACHE_TTL.AQI_HISTORICAL;
